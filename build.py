...
parser.add_argument("-m", "--mode", choices=("build", "clean", "docker", "docker-continue"), default="build",
                    help="build/rebuild all, clean and rebuild just the last stage,"
                          " build using Docker, or continue from a failed build with Docker")
...
if __name__ == "__main__":
    ...
    if args.skip_stages:
        ...
        skip_files = " ".join(["{base_dir}/stage{stage}/SKIP".format(base_dir=config["BASE_DIR"], stage=stage)
                               for stage in args.skip_stages])

        system("touch " + " ".join(skip_files))
    else:
        skip_files = None
